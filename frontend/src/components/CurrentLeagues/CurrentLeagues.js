                            ...
                            {league.portfolioList
                                && league.portfolioList.map((portfolio) => {
                                    ...
                                    if (portfolio.owner === username) {
                                        ...
                                        return (<p className="current-league-worth">${portfolio.currentNetWorth}({portfolio.currentNetWorth / league.settings.balance > 1 ? `+${parseFloat((portfolio.currentNetWorth / league.settings.balance - 1) * 100).toFixed(2)}%` : `-${parseFloat((1 - portfolio.currentNetWorth / league.settings.balance) * 100).toFixed(2)}%` })</p>);
                                    }
                                    console.log('NOne');
                                    return null;
                                })}
